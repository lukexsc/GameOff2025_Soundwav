...
public static class SerializedPropertyExtensions
{
    ...
    public static T GetNestedObject<T>(string path, object obj, bool includeAllBases = false)
    {
        ...
        if (splitPath.Length > 2 && splitPath[1] == "Array") // If it's an Array or List, handle here
        {
            BindingFlags bindings = BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic;

            // Get Data Index
            int index;
            int.TryParse(splitPath[2].Substring(5, 1), out index);

            // Check List
            FieldInfo field = obj.GetType().GetField(splitPath[0], bindings);
            try
            {
                List<T> listTest = (field.GetValue(obj) as IEnumerable).OfType<T>().ToList();
                if (listTest != null) return (T)listTest[index];
            }
            catch { }
            ...
            try
            {
                T[] arrayTest = (T[])field.GetValue(obj);
                if (arrayTest != null) return arrayTest[index];
            }
            catch { }
        }

        foreach (string part in splitPath)
        {
        ...
        BindingFlags bindings = BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
    {
        ...
        if (includeAllBases)
        {
            foreach (System.Type type in GetBaseClassesAndInterfaces(obj.GetType()))
            {
    ...
    public static IEnumerable<System.Type> GetBaseClassesAndInterfaces(this System.Type type, bool includeSelf = false)
    {
